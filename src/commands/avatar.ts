...
export const avatarCommand = new SlashCommandBuilder()
  ...
  .addStringOption((option) => option.setName("name").setDescription("Avatar name.").setRequired(true).addChoices(...AVATARS.map((avatar) => ({ name: avatar, value: avatar }))));
...
export async function handleAvatar(interaction: ChatInputCommandInteraction) {
  ...
  const qualified = result.qualified.map((entry, index) => {
    ...
    return `${index + 1}. ${entry.displayName}, ${entry.points} pts, ${formatRecord(entry.wins, entry.losses, entry.draws)}, ${formatPercent(winrate)}`;
  });

  const notQualified = result.notQualified.map((entry) => `- ${entry.displayName}, ${entry.points} pts, ${formatRecord(entry.wins, entry.losses, entry.draws)}`);
